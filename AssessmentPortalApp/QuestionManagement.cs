    ...
    public partial class QuestionManagement : Form
    {
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                selectedQuestionId = Convert.ToInt32(row.Cells["qid"].Value); // Store the selected question ID
                ...
                // Populate the subject in cmbChooseSubject
                ...
                private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    string query = @"
                        UPDATE QuestionBank
                        SET subjectname = @Subjectname, question = @Question, optionA = @OptionA, optionB = @OptionB, optionC = @OptionC, optionD = @OptionD, correctanswer = @CorrectAnswer
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        command.Parameters.AddWithValue("@Subjectname", cmbChooseSubject.SelectedValue);
                        command.Parameters.AddWithValue("@Question", txtQuestion.Text);
                        command.Parameters.AddWithValue("@OptionA", txtOptionA.Text);
                        command.Parameters.AddWithValue("@OptionB", txtOptionB.Text);
                        command.Parameters.AddWithValue("@OptionC", txtOptionC.Text);
        ...
        {
            string searchText = txtSearch.Text.Trim(); // Get the search text from txtSearch
            LoadQuestions(searchText: searchText); // Pass the search text to the LoadQuestions method
        }
    }

...
namespace AssessmentPortalApp
{
    public partial class SubjectManagementAdmin1 : Form
    {
        ...
        private void SubjectManagementAdmin1_Load(object sender, EventArgs e)
        {
            ...
            BindGrid();              // Load sub 
            BindGridAllocation(); // Load allocated sub into the grid
            LoadStaff();
            LoadSubject();
            ...
            using (SqlCommand cmdSave = new SqlCommand())
            {
                ...
                try
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        LoadSubject();  // Refresh the subject dropdown
                        BindGrid();     // Refresh the subject grid
                    }
                    else
                    {
        ...
        private void BindGridAllocation()
        {
            ...
            // Update column headers to match the required fields
        ...
        private void cmbSelectStaff_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            // Check if the selected staff member has any subjects allocated
            ...
            if (allocatedSubjectCount > 0)
            {
                ...
                cmbSelectSubject.Refresh(); // Force the dropdown to refresh its visibility state
            ...
            else
            {
                ...
                cmbSelectSubject.Refresh(); // Refresh visibility state
            }
        ...
        private void btnAllot_Click(object sender, EventArgs e)
        {
            ...
            // Check if the selected staff member is already assigned to a subject
            ...
            // Proceed with allocation if validation passes
            SqlCommand cmd = new SqlCommand(
            ...
            try
            {
                if (cmd.ExecuteNonQuery() > 0)
                {
                    ...
                    // Clear the cmbSelectStaff selection by setting it to "Select a staff"
                    cmbSelectStaff.SelectedIndex = 0;

                    LoadSubject(); // Refresh the subject dropdown
                    BindGridAllocation(); // Refresh the allocations grid
                }
                else
                {
        ...
        private void pictureBox2_Click(object sender, EventArgs e)
        {
            ...
            idToBeEdited = 0;       // Reset the ID to be edited
            action = 1;             // Reset the action to the default 'Add' mode
            txtSubject.Text = "";   // Clear the subject text field
            txtSubject.Enabled = true;  // Enable the subject text field
            btnSave.Text = "Save";  // Set the button text to "Save"
                 // Clear any message text

            LoadSubject();          // Refresh the subject dropdown
            BindGrid();             // Refresh the subject grid to show any updates
        }
    }
}

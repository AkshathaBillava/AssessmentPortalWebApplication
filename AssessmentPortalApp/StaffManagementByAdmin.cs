...
namespace AssessmentPortalApp
{
    public partial class StaffManagementByAdmin : Form
    {
        ...
        SqlConnection con;
        public int adminUserId; // Admin user ID
        public string userName;
        public int roleId; // Admin role ID = 1
        ...
        private void StaffManagementByAdmin_Load(object sender, EventArgs e)
        {
            ...
            lblId.Text = "0"; // Reset staff user ID
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            // Check if the current operation is for a staff member
            if (selectedRoleId != 2)
            {
            ...
            string query;
            int staffUserId = Convert.ToInt32(lblId.Text); // Local variable for staff user ID

            if (staffUserId == 0)  // Adding a new staff member
            {
                query = "INSERT INTO AllUsers (name, mblno, email, gender, roleid, statusid) " +
                        "VALUES (@name, @mblno, @email, @gender, @roleid, @statusid); " +
                        "SELECT SCOPE_IDENTITY();";
            }
            else  // Updating an existing staff member
            {

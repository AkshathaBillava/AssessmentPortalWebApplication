    ...
    public partial class AllUsersLoginPage1 : Form
    {
        ...
        private void btnLogin_Click_1(object sender, EventArgs e)
        {
            ...
            if (result.HasRows)
            {
                ...
                {
                    // Correct email and password, proceed to login
                    if (roleid == 1)
                    {
                        AdminHomePage admin = new AdminHomePage();
                        admin.userId = userid;
                    ...
                    // Code for cancel button in ChangePasswordPage
                }
                else
                {
                    // Mobile number is incorrect for the existing email
                    errorProvider1.SetError(txtMblNo, "Incorrect mobile number for the given email.");
                    errorProvider1.SetError(txtEmail, string.Empty); // Clear email error
                }
            }
            else
            {
                ...
                errorProvider1.SetError(txtMblNo, string.Empty); // Clear mobile number error
            }
